...
def getConvResponse():
    ...
    response = assistant.message(workspace_id='c059198d-c313-45e1-9375-612b1adeda2b',
                                 input={'text': convText},
                                 context=jsonContext)
...
def getSpeechFromText():
    ...
    authenticatortextto = IAMAuthenticator('0aQebO9pT7JJiPU3pGLPLit83BtGyRmMS5EXTixN9rgF')

    ttsService = TextToSpeechV1(
        authenticator = authenticatortextto
    )
    ttsService.set_service_url('https://stream.watsonplatform.net/text-to-speech/api')
...
def getTextFromSpeech():
    ...
    authenticatorspeech = IAMAuthenticator('JrEueDywqylVUJdApPydLFFg9JEYQTrKKX6I6ktDXpvi')
    sttService = SpeechToTextV1(
        authenticator= authenticatorspeech
    )
    sttService.set_service_url('https://stream.watsonplatform.net/speech-to-text/api')
...
if __name__ == "__main__":
    ...
    assistant = AssistantV1(version="2019-12-13", authenticator=IAMAuthenticator("7ypN9ZgqcmmHQUrJHrvhbVDMl_kW2gJn7qRzBl7B9-HY"))
